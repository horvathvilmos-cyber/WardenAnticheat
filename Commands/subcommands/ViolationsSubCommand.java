...
public class ViolationsSubCommand implements SubCommand {
    ...
    public void execute(CommandSender sender, String[] args) {
        if (args.length < 2) {
            sender.sendMessage(ChatColor.RED + "Használat: " + getUsage());
            return;
        }

        Player target = Bukkit.getPlayer(args[1]);
        if (target == null) {
            sender.sendMessage(ChatColor.RED + "[Warden] A játékos nem található vagy nincs online!");
            return;
        }

        PlayerData data = PlayerDataManager.getData(target);
        if (data == null) {
            sender.sendMessage(ChatColor.RED + "[Warden] Nem található adat ehhez a játékoshoz!");
            return;
        }
        ...
        if (data.getAllViolations().isEmpty()) {
            sender.sendMessage(ChatColor.GRAY + "Nincs egyetlen flag sem.");
            return;
        }
